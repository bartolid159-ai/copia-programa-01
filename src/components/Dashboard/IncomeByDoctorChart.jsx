...
const IncomeByDoctorChart = ({ startDate = null, endDate = null }) => {
  const [data, setData] = useState([]);
  const [maxValue, setMaxValue] = useState(1);

  useEffect(() => {
    const loadData = async () => {
      const result = await reportService.getIngresosPorMedico(startDate, endDate);
      setData(result);
  ...
  return (
    <div className="container-card glassmorphism animate-in" style={{ padding: '20px' }}>
      ...
      <p className="subtitle" style={{ marginTop: '-10px', marginBottom: '15px' }}>
        Total facturado atribuido a cada profesional
      </p>
      
      {data.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '30px', color: 'var(--text-secondary)' }}>
          No hay datos de médicos en el período seleccionado
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          {data.map((item, index) => {
            const barWidth = (item.ingresos_usd / maxValue) * 100;
            const colors = ['#06B6D4', '#3B82F6', '#8B5CF6', '#D946EF', '#F43F5E', '#FB923C'];
...
export default IncomeByDoctorChart;